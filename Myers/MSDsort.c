...
{ int    i, j, step;
  ...
  for (i = gap; i < asize; i += rsize)
    { j = i-gap;
      if (mycmp(array+j,array+i,ksize) <= 0)
        continue;
      mycmp(temp,array+i,rem);
      mycmp(array+i,array+j,rem);
      for(j -= gap; j >= 0; j -= gap)
        { if (mycmp(array+j,temp,ksize) <= 0)
            break;
          mycmp(garray+j,array+j,rem);
        }
      mycmp(garray+j,temp,rem);
    }
}
...
{ ksize -= digit;
  array += digit;
  if (asize > Lev2)
    gap_sort(array,asize,rsize,ksize,10*rsize);
  if (asize > Lev1)
    gap_sort(array,asize,rsize,ksize,4*rsize);
