...
data, process intermediates, and the ultimate reconstructed genome assemblies.  We have a core set of ASCII file types based on very simple principles, then specific derived types that add additional constraints to represent specific types of data sets.
For example, the
VGP currently collects 3 core sequence data sets for a given genome project which can be encoded in subtypes of the sequence file type.  by convention file types and subtypes are three letter lower case strings, which can be used as filename suffixes, e.g. dataset1.seq.

**seq**   primary file type for sequence objects

**irp**	subtype for encoding Illumina read pairs.  We use this to encode data generated by a Hi-C protocol, but this applies more generally to any read pair protocol.

**pbr**	subtype for encoding PacBio long reads and relevant meta-data.

**10x**	subtype for encoding 10X Genomics read clouds with their extracted barcodes. 

Other core file types are

**rmd**   primary type for restriction map digests.  We used this for primary BioNano .bnx data, but also for derived restriction maps either generated by assembly from primary data, or by *in silico* digestion from sequences.

**sxs**   primary type for sequence alignments (Sequence Cross Sequence, pronounced like "success").  We also use a subtype of this **rxr** for restriction map comparisons, and another subtype **map** for representing the relationship between a set of consensus sequences and the reads or other sequences from which they were derived.

**.scf** a collection of scaffolding suggestions representing putative joins and breaks between sequences in some **seq** file.

Finally there is a utility file type

**.lis** this can be used to record arbitrary lists of objects in any of the VGP file types.
...
in producing the current file.  Each line contains 3 strings and 2 integers giving (a) the program name, (b) the version of that program as a string, (c) the command line that was executed, and (d) the date and time it
...
```
Since there can be multiple provenance lines in a header, indicating the sequence of operations that generated the file, one expects to see a count line

```
	# ! <int:num_provenance_lines>
```
before the provenance lines in order to know how many there are.
