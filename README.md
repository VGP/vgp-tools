...
**seq**   primary file type for sequence objects. There are multiple subtypes of this including **irp** for Illumina read pairs, which we use to encode Hi-C read data but which can be applied more generally to any read pair protocol; **pbr** for PacBio long reads and relevant meta-data, **10x**	for 10X Genomics read clouds with their extracted barcodes;
