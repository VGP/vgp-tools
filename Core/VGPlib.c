 ...
 *
 * HISTORY:
 * Last edited: Apr 16 23:19 2020 (rd109)
...
{ va_list args;
  ...
  fprintf (stderr, "VGP PARSE ERROR ");
...
{ eatWhite (vf);
  if (vf->isCheckString)
    ...
    die ("VGP parse error: failed to read %d byte string", n);
}
...
{ BOOL      isAscii;
  ...
  if (vf->isWrite)
    die ("VGP read error: trying to read a line from a file open for writing");
  if (vf->isFinal)
    die ("VGP read error: cannot read more data after counts are finalized");
          ...
          if (listLen > 0)
            { if (li->fieldSpec[ix] == STRING_LIST) // handle as ASCII
                ...
                { if (fread (&nBits, sizeof(I64), 1, vf->f) != 1)
                    die ("VGP read error: fail to read list nBits");
                  if (fread (vf->codecBuf, ((nBits+7) >> 3), 1, vf->f) != 1)
                    die ("VGP read error: fail to read compressed list");
                ...
                { usedBytes = li->listByteSize - (vgpInt(vf,ix) >> 56); // data bytes
                  ...
                  if ((I64) fread (li->buffer, 1, listSize, vf->f) != listSize)
                    die ("VGP read error: list read %lld not %lld", x, listSize);
                }
...
#define OPEN_ERROR1(x) { fprintf (stderr,"VGP file error: %s\n",x); fclose(f) ; return NULL; }
#define OPEN_ERROR3(x,y,z) { fprintf (stderr,"VGP file error: ") ; \
	...
	ungetc(c,f);
      ...
      { if (fscanf(f," %d %s %d %d",&slen,name,&major,&minor) != 4)
	  OPEN_ERROR1("line 1: failed to read 4 items") ;
        while (getc(f) != '\n')
	  if (feof(f))
	    OPEN_ERROR1("end of file before end of line 1") ;

        if (major != MAJOR_NUMBER)
          OPEN_ERROR3("major version file %d != code %d", major, MAJOR_NUMBER) ;
        if (minor > MINOR_NUMBER)
          OPEN_ERROR3("minor version file %d > code %d", major, MAJOR_NUMBER) ;
	  ...
	  OPEN_ERROR3("primary fileType mismatch file %s != %s", name, fileTypeName[fileType]);
  ...
  if (nthreads == 1)
    ...
    { VgpFile *v;
      ...
      if (strcmp (path, "-") == 0)
        die ("VGP error: parallel input incompatible with stdin as input");
      ...
      { case '1':
        ...
        case '$':  // read footer - goto end, find offset to start of footer and go there
          ...
          if (fseek (f, -sizeof(off_t), SEEK_END) != 0)
            die ("VGP file error: can't seek to final line");

          if (fread (&footOff, sizeof(off_t), 1, f) != 1)
            die ("VGP file error: can't read footer offset");

          if (fseeko (f, footOff, SEEK_SET) != 0)
            die ("VGP file error: can't seek to start of footer");
          break;

        case '^':    // end of footer - return to where we jumped from header
          if (fseeko (f, startOff, SEEK_SET) != 0)
            die ("VGP file error: can't seek back");
          break;
    ...
    for (i = 1; i < nthreads; i++)
      { v = vf+i;
        ...
        if (fseeko (v->f, startOff, SEEK_SET) != 0)
          die ("VGP file error: can't seek to start of data");
...
{ VgpFile *vf, *v;
  ...
  if (subType && subPrimary[subType] != fileType)
    { fprintf (stderr, "VGP open error: subtype %s is not secondary for filetype %s",
    ...
    { v = vgpFileCreate (fileType);
      ...
      if (i > 0)
        { v->codecTrainingSize /= 3*nthreads;
          ...
          f = fopen (name, "w");
          if (f == NULL)
            die ("VGP file error: cannot create temporary file %d for parallel write", i);
        }
      else
        { v->share = nthreads;
...
{ int         i,n;
  ...
  if ( ! vf->isWrite)
    die ("VGP error: trying to write header to a file open for reading");
  if (vf->line > 0)
    die ("VGP error: cannot write header after writing one or more data lines");
  if (vf->lineInfo[(int) vf->objectType]->given.count == 0 && ! vf->isBinary)
    die ("VGP error: information for ASCII header is not present, use vgpFileOpenWriteFrom");
...
{ I64       i, j, ix;
  ...
  if ( ! vf->isWrite)
    die ("VGP write error: trying to write a line to a file open for reading");
  if (vf->isFinal && isalpha(t))
    die ("VGP write error: annot write more data after counts are finalized %c" ,t);

  li = vf->lineInfo[(int) t];
  if (li == NULL)
    die ("VGP write error: line type %c not present in file spec %s ",
  ...
  if (ix >= 0)
	...
	die ("VGP write error: listLen %lld must be positive", listLen) ;
    }
...
#define CHECK_INDEX
	      ...
	      die ("VGP write error: byte offset index error") ;
	  }
#endif
          vf->object += 1;
        }
            ...
            { fputc (x, vf->f);
              if (nField > 0)
                { if (fwrite (vf->field, fieldSize, 1, vf->f) != 1)
                    die ("VGP write error: write fields: t %c, nField %d, fieldSize %lld",
			 t, nField, fieldSize);
                }
	      vf->byte += 1 + fieldSize ;
            }
        ...
        { li->accum.total += listLen;
		  ...
		  vf->byte += sizeof(I64) + ((nBits+7) >> 3);
                ...
                { if (fwrite (listBuf, listSize, 1, vf->f) != 1)
                    die ("VGP write error: failed to write list ix %d listLen %lld listSize %lld listBuf %lx",
        ...
        { case INT:
          ...
          case STRING_LIST:
            ...
            if (li->fieldSpec[i] == STRING)
              { if (listLen > INT_MAX)
                  die ("VGP write error: string length %lld > current max %d", listLen, INT_MAX);
...
{ int       i,n;
  ...
  footOff = ftello (vf->f);
  if (footOff < 0)
    die ("VGP write error: failed footer ftell");
  ...
  if (fwrite (&footOff, sizeof(off_t), 1, vf->f) != 1)
    die ("VGP write error: failed writing footer offset");
}
...
{
  if (vf->share < 0)
    die ("VGP file error: annot call vgpFileClose on a slave VgpFile");
            ...
            { fclose (vf[i].f);
              ...
              while ((nread = read(fid,buf,10000000)) > 0)
                if ((int) fwrite(buf,1,nread,vf->f) != nread)
                  die ("VGP write error: while cat'ing thread bits (vgpFileClose)");
              if (unlink(name) < 0)
                die ("VGP write error: could not delete thread file %s", name);
            }
          free(buf);
        }
